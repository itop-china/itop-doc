...
date: 2022-11-09
weight: 5
description: "介绍了对象的常用增删改查、状态变化等操作，以及打印等功能。"
---

{{% pageinfo %}}

在iTop中，操作用于修改对象。操作可以执行：

- 在对象列表（参考章节 [管理列表](https://www.itophub.io/wiki/page?id=3_2_0:user:lists)）情况下，所选的操作将应用于列表中的所有对象
- 在对象的详细页面
...
_默认情况下，该操作总是会显示为图标_，因此在下拉菜单中不会出现。

>> ![提示](../../images/tip.png) 根据上下文，有些字段可能会自动预填充。例如，如果上下文设置为 **Demo** 组织，当点击 **新建** 创建一个新的个人的时候，将显示一个带有组织的表单，预填充为 *Demo*。

## 修改

此操作显示要更新对象的表单。创建表单和修改表单两者非常相似，只是在修改对象时，表单中的值会预先填充为当前要修改对象的值。

_默认情况下，此操作一直显示为图标，因此不会在下拉菜单中出现。_
...
在这种情况下，iTop发现有几个人属于本组织。由于数据模型的定义要求个人必须属于一个组织，而且个人的删除不是自动的，因此当存在属于组织的个人时，不能删除组织。同样的逻辑适用于所有必须具有组织的其他对象。
...
根据所显示对象的类型，还提供了额外的 **关系** 菜单。例如，对于大多数配置项，定义了两类关系： *影响* 和 *依赖于* 。这种情况下，**其他操作** 弹出菜单中会有两个额外可用的菜单。

当选择这样的操作时，会显示关系的页面：

- 您可以使用顶部带有图标的复选框来过滤显示（更新选择后只需点击 **刷新** 即可）。
- 将鼠标移到到对象上以显示附加信息的工具提示（以及指向其详细信息的链接）
- 在对象上单击并拖动，可重新排列图表
- 点击并拖动 *背景* 可以移动整个图表
- **+** 和 **-** 按钮，以及图表右上方的 **滑块**（和鼠标滚轮）用于调整缩放级别
- **分组阈值** 定义了用于创建一个组的相似对象（相同类型，相同状态）的最小数量，以取代图表中的单个对象，简化图表。更改此值时，单击 **刷新** 将以新的分组级别重新计算图表

>> ![提示](../../images/tip.png) 通过右键单击图表中的对象，可以从弹出菜单中选择 **详情** 返回其详细信息。对于对象的 **组**，**详情菜单** 显示该组中的元素列表。

### 上下文

- **其他信息** 下拉列表定义了在计算和显示图表时对于 **工单**（更新选择后只需按 **刷新**）的考虑。
...
>> ![警告](../../images/warning.png) 当勾选了 **包含的对象列表** 并且要导出的对象列表很大时，PDF文档的创建可能会非常慢（> 100 对象）。
...
一个对象的CSV导出类似于 [列表的CSV导出](https://www.itophub.io/wiki/page?id=3_2_0:user:lists#csv_export)，除非结果导出中只有一行数据。

## Excel 导出

导出一个对象的Excel类似于 [列表的Excel导出](https://www.itophub.io/wiki/page?id=3_2_0:user:lists#excel_export)，除非结果导出中只有一行数据。
...
以下是由 [用户操作配置器](../../extensions/itop-object-copier/) 扩展的示例：

>> [对象显示和修改](../04-display_and_modification_of_an_object/)  ← 上一篇 | 下一篇 → [搜索](../06-search/)

---
原文：<https://www.itophub.io/wiki/page?id=3_2_0:user:actions>

版本：3_2_0/user/actions.txt · Last modified: 2024/09/10 10:25 by 127.0.0.1
